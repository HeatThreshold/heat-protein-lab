...
1. **Rotate the Stitch API key.** This is operator-only — Claude Code can't sign into Google Cloud Console with operator credentials. Confirm rotation in the next session log entry.
2. **Re-run Chapter 4 Stitch generation** to produce the missing HTML mockup.
3. **Begin Phase 0 smoke tests** for the seven Science Skills the project will use (PDB, AlphaFold, HPA, PubMed, ClinVar, dbSNP, Reactome). This is the exit criterion for Phase 0 and the trigger for the Beat 1 post.
